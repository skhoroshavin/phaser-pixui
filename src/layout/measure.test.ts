...
describe("measure", () => {
  ...
  it("auto-sizes nested containers to a leaf's margin box, explicit size overriding intrinsic", () => {
    ...
    expect(outer.rect).toEqual({ x: 0, y: 0, w: 82, h: 56 });
    expect(inner.rect).toEqual({ x: 6, y: 3, w: 70, h: 50 });
    // leaf inset by inner's + its own margins; width overridden to 50, height intrinsic 40
    expect(leaf.rect).toEqual({ x: 16, y: 8, w: 50, h: 40 });
  });
  ...
  it("auto-sizes a flex container to its children's margin boxes", () => {
    ...
    expect(flex.rect).toEqual({ x: 0, y: 0, w: 70, h: 94 });
  });
  ...
  it("uses a closure intrinsic's returned size, passing undefined when nothing limits the width", () => {
    const root = viewport(200, 200);
    root.add(
      new Node({ layout: {}, intrinsicSize: (aw) => ({ w: aw === undefined ? 50 : 0, h: 30 }) }),
    );
    resolve(root);
    // closure's returned size is used (h: 30 comes through); it received
  ...
  it("flows a stretched leaf's wrapped height up to an auto-height parent", () => {
    ...
    container.add(
      new Node({
        ...
        intrinsicSize: (aw) => ({ w: 0, h: aw === 200 ? 40 : 0 }),
      }),
    );
    root.add(container);
    resolve(root);
  ...
  it("wraps stretched text at a maxWidth-capped container's width", () => {
    ...
    frame.add(
      new Node({
        layout: { left: 0, right: 0 },
        intrinsicSize: (aw) => ({ w: 0, h: aw === 224 ? 48 : 16 }),
      }),
    );
    root.add(frame);
    resolve(root);
    expect(frame.rect.w).toBe(224);
    expect(frame.rect.h).toBe(48);
  });

  it("stretches, not centers, an auto-width element with both edges and auto margins", () => {
    const root = viewport(320, 240);
    root.add(
      new Node({ layout: { left: 0, right: 0, marginX: "auto" }, intrinsicSize: { w: 50, h: 10 } }),
    );
    resolve(root);
    expect(root.children[0]!.rect).toEqual({ x: 0, y: 0, w: 320, h: 10 });
  });
  ...
  it("caps the width before auto-margin centering", () => {
    ...
    expect(root.children[0]!.rect).toEqual({ x: 120, y: 0, w: 80, h: 0 });
  });
  ...
  it("re-wraps a stretched flex item's width-dependent content in a definite-width column", () => {
    ...
    // text that wraps taller (h:40) when given the stretched width 100,
    // otherwise a single line (h:20)
    const text = new Node({ intrinsicSize: (aw) => ({ w: 0, h: aw === 100 ? 40 : 20 }) });
    col.add(text);
    root.add(col);
    resolve(root);
    ...
    expect(text.rect).toEqual({ x: 0, y: 0, w: 100, h: 40 });
    expect(col.rect).toEqual({ x: 0, y: 0, w: 100, h: 40 });
  });
  ...
  it("wraps stretched text at the margin-reduced content width, not the full edge-stretch width", () => {
    ...
    root.add(
      new Node({
        ...
        intrinsicSize: (aw) => ({ w: 0, h: aw !== undefined && aw <= 100 ? 40 : 10 }),
      }),
    );
    resolve(root);
    // text wrapped at 100 (not 200) → height 40; rect width = 100, inset by the 50 margin
    expect(root.children[0]!.rect).toEqual({ x: 50, y: 0, w: 100, h: 40 });
  });
});
